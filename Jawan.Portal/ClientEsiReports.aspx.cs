...
{
    public partial class ClientEsiReports : System.Web.UI.Page
    {
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            SPName = "RepotForESIDetailsofEmployeesBasedonclient";
        ...
        {
            gve.Export("ESIReport.xls", this.GVListEmployees);
        }


        float totalESIDuties = 0;
        float totalESIWAGES = 0;
        float totalESI = 0;
        float totalESIEmpr = 0;
        float totalTotalESI = 0;

        protected void GVListEmployees_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                float ESIDuties = float.Parse(((Label)e.Row.FindControl("lblESIDuties")).Text);
                totalESIDuties += ESIDuties;
                ...
                float TotalESI = float.Parse(((Label)e.Row.FindControl("lblTotalESI")).Text);
                totalTotalESI += TotalESI;
            }
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                ((Label)e.Row.FindControl("lblTotalESIDuties")).Text = totalESIDuties.ToString();
                ...
                ((Label)e.Row.FindControl("lblTotalTotalESI")).Text = totalTotalESI.ToString();
